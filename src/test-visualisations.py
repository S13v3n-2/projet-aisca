...
if submit:
    ...
    if len(user_text.split()) < 30:
        st.warning(
            "⚠️ Réponses trop courtes. Veuillez détailler davantage vos expériences et aspirations pour une analyse fiable.")
    else:
        ...
        df_scores = pd.DataFrame([
            {
                ...
                "Score": f"{b['score']:.0%}",
                "Niveau": "🟢 Maîtrisé" if b['score'] >= 0.7 else "🟡 Intermédiaire" if b[
                                                                                          'score'] >= 0.4 else "🔴 À développer"
            }
            for b in top_jobs[0]['blocs']
        ...
        with st.expander("🔍 DEBUG : Texte utilisateur généré", expanded=False):
            ...
            if juridique_boost:
                st.success(
                    f"⚖️ Boost juridique actif : {len(juridique_boost.split())} mots injectés (B01 compétences × répétitions)")
            else:
